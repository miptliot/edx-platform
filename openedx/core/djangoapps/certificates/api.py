...
from lms.djangoapps.certificates.models import CertificateWhitelist, CertificateBlacklist
...
def can_show_certificate_message(course, student, course_grade, certificates_enabled_for_course):
    ...
    user_in_blacklist = CertificateBlacklist.objects.filter(user=student, course_id=course.id).exists()
